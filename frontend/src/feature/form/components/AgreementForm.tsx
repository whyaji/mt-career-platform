...
export function AgreementForm({ form, isMobile = false, batch }: AgreementFormProps) {
  return (
    <Paper
      p={isMobile ? 'sm' : 'xl'}
      radius="lg"
      ...
      <Stack gap="lg">
        <div>
          <Title order={2} mb="xs" c="orange">
            {batch.program_category?.name ?? 'CBI GROUP'}
            {batch.number_code ? ` ANGKATAN ${batch.number_code}` : ''}
          </Title>
