...
import { Route } from '@/routes/talenthub/_authenticated/open-programs';
import type { BatchType } from '@/types/batch.type';

export function OpenProgramListScreen() {
  ...
  const handleViewApplicants = (batch: BatchType) => {
    navigate({
      to: '/talenthub/open-programs/$batchId',
      ...
      render: (_id: unknown, record: BatchType) => (
        <Group gap="xs">
          <Tooltip label="View Applicant">
            <Link to={`/talenthub/open-programs/${record.id}` as never}>
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <DefaultTable
        title="Open Programs"
        description="Manage and view open program batches"
        columns={columns}
        ...
        emptyMessage="No open programs found"
